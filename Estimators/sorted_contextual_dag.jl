...
function train(model, lambda, params, x, z, x_val, z_val, optimiser, epoch_max, early_stop, patience, 
    ...
    function objective(model; x = x, z = z, par = lambda, params = params, inference = false, mask = mask_train)
        p, n = size(x)
    ...
    val_loss = objective(model, x = x_val, z = z_val, par = theta[], inference = true, mask = mask_valid)
    epochs = 0
    ...
    val_nonzero = sum(project(model(z_val) .* mask_valid, par = theta, params = params, inference = true)[1] .!= 0) / n_val
