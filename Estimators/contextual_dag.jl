# =================================================================================================#
# Description: Implementation of the contextual DAG
# Author: Ryan Thompson
# =================================================================================================#

module ContextualDAG
...
function train(model, lambda, params, x, z, x_val, z_val, optimiser, epoch_max, early_stop, patience, 
    verbose, verbose_freq, zero_inds)
    ...
    function objective(model; x = x, z = z, par = lambda, params = params, inference = false)
        p, n = size(x)
        ŵ, theta[] = project(model(z), par = par, params = params, inference = inference)
        x̂ = xw_mult(ŵ; x = x)
        sum((x - x̂) .^ 2) / (n * p)
    end
...
function init(model, x, z, x_val, z_val, optimiser, epoch_max, early_stop, patience, verbose, 
    ...
    # Create objective function
    function objective(model; x = x, z = z)
        p, n = size(x)
        ŵ = model(z)
        x̂ = xw_mult(ŵ; x = x)
        sum((x - x̂) .^ 2) / (n * p)
    ...
    # Initialise variables
    val_loss = objective(model, x = x_val, z = z_val)
